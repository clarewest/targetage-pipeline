...
unnested_associations <- associations %>% select(morbidity, targetId, overallDatatypeHarmonicVector) %>% unnest(overallDatatypeHarmonicVector)
wide_associations <- unnested_associations %>% select(-weight, -datatypeEvidenceCount) %>% pivot_wider(names_from = datatypeId, values_from = datatypeHarmonicScore) %>% left_join(unnested_associations %>% filter(datatypeId=="literature") %>% select(targetId, morbidity, literatureEvidenceCount = datatypeEvidenceCount))
genetic_associations <- unnested_associations %>% filter(datatypeId=="genetic_association", morbidity %in% d) %>% select(morbidity, targetId, datatypeHarmonicScore) %>% mutate(datatypeHarmonicScore = round(datatypeHarmonicScore, 2)) %>% pivot_wider(names_from = "morbidity", values_from = "datatypeHarmonicScore") 
literature_counts <- unnested_associations %>% filter(datatypeId=="literature") %>% select(morbidity, targetId, literatureEvidenceCount = datatypeEvidenceCount)
max_literature_counts <- literature_counts %>% group_by(targetId) %>% slice(which.max(literatureEvidenceCount)) %>% rename(maxLiteratureEvidencePhenotype = morbidity, maxLiteratureEvidenceCount = literatureEvidenceCount)

save(genetic_associations, file = "TargetAge/data/associations.Rda")
...
target_annotations <- targets %>% select(targetId, targetSymbol, bioType, hgncId, chemicalProbes, symbolSynonyms, tractability, safety, tep) %>% jsonlite::flatten() %>% left_join(max_literature_counts)
save(target_annotations, file = "TargetAge/data/target_annotations.Rda")
...
l2g_all_joined <- l2g_all_joined %>% left_join(targets %>% select(gene.id = targetId, targetSymbol))
...
top_cluster_genes_summarised <- top_cluster_genes %>% filter(yProbaModel >= 0.5) %>% ungroup() %>% group_by(gene.id, targetSymbol, cluster) %>% count() %>% filter(!is.na(cluster)) %>% ungroup() %>% group_by(gene.id, targetSymbol) %>% summarise(n_clusters = length(cluster)) %>% arrange(-n_clusters)

top_cluster_genes_summarised %>% ungroup() %>% count(n_clusters) %>% mutate(prop = n/sum(n)*100)
...
## Compare sets of genes in GenAge, CellAge, TargetAge
load("TargetAge/data/genage.Rda")
load("TargetAge/data/cellage.Rda")
...
go_compare <- go %>%
  bind_rows(go %>% filter(targetId %in% targetage) %>% mutate(group = "targetage")) %>%
  group_by(group, targetId, targetSymbol, goHallmarkTerm) %>% count() %>% pivot_wider(names_from = goHallmarkTerm, values_from = n, values_fill = 0) %>% rename(other = `NA`) %>% pivot_longer(-c("group", "targetId", "targetSymbol"))

## number of targets with at least one annotation in each hallmark
go_compare_hallmarks <- go_compare %>% ungroup() %>% group_by(group) %>% mutate(total = length(unique(targetId))) %>% ungroup() %>% group_by(group, total, name) %>% summarise(n = sum(value>0)) %>% mutate(proportion = n/total)

## bar plot 
## but need to make sure I'm only using >0.5 genes 
hallmarks_bar <- ggplot(subset(go_compare_hallmarks, name!="other"), aes(x=proportion, y = reorder(name, proportion), fill = group, label = paste0(" ", scales::percent(proportion, accuracy = 0.1)))) + geom_col(position = "dodge") + theme_bw() + geom_text(position = position_dodge(width = 0.9), hjust = 0, size = 3) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text.y = element_text(size = 10, colour = "black"), axis.title.y = element_blank(), legend.position = c(0.8, 0.1), legend.title = element_blank()) + scale_x_continuous(limits = c(0, 0.15), labels = scales::percent_format(accuracy = 1), expand = c(0, NA)) + labs(x = "Percentage of targets", subtitle = "Proportion of targets mapped to\nHallmarks of Ageing GO terms")

interesting_pathways <- data.frame(reactome.term = c("Autophagy", "Cell Cycle", "Cell-Cell communication", "DNA Repair", "Extracellular matrix organization", "Metabolism", "Metabolism of proteins"), reactome = c("R-HSA-9612973", "R-HSA-1640170", "R-HSA-1500931", "R-HSA-73894", "R-HSA-1474244", "R-HSA-1430728", "R-HSA-392499" ))
reactome <- targets %>% select(targetId, targetSymbol, reactome) %>% unnest(reactome) %>% left_join(interesting_pathways)

## See overlap of gene sets
topgenes_genage_cellage <- top_cluster_genes_summarised %>% full_join(genage, by = c("gene.symbol" = "target.gene_info.symbol")) %>% full_join(cellage, by = c("gene.symbol" = "symbol"))

## Prepare for enrichment analysis
gene_sets_entrez <- list(TargetAge = top_cluster_genes_summarised$gene.id,
                  GenAgeHuman = genage$entrez.gene.id %>% unique(),
                  CellAge = cellage$entrezid %>% unique())

save(gene_sets, file = "gene_sets.Rda")
save(gene_sets_entrez, file = "gene_sets_entrez.Rda")
...
ggsave(venn, file = "genage_cellage_venn.png", width = 6, height = 6, dpi = 300)

#(genage_set %in% targetage_set %>% sum())/length(genage_set)*100
#(cellage_set %in% targetage_set %>% sum())/length(cellage_set)*100

#sum(genage_set %in% targetage_set) / length(unique(c(genage_set, targetage_set)))
...
ggsave(fig, file = "enrichment_figure.png", width = 8, height = 9, dpi = 300)
ggsave(fig, file = "enrichment_figure.pdf", width = 14, height = 13)
...
targetage_tract <- targets %>% filter(targetId %in% targetage) %>% jsonlite::flatten()
tract_plot_data <- targetage_tract %>% mutate(tractability.other_modalities.top_category = ifelse(tractability.other_modalities.categories.clinical_precedence > 0, "Clinical_Precedence", NA)) %>% select(targetId, ends_with("top_category")) %>% pivot_longer(-targetId, names_to = "modality", values_to = "top_category") %>% count(modality, top_category) %>% mutate(modality = gsub(".*[.]([^.]+)[.].*", "\\1", modality), top_category = gsub("_", " ", top_category)) %>% mutate(top_category = gsub("le ab", "le \n", top_category), top_category = gsub(" sm", "", top_category), top_category = gsub(" ab", "", top_category), top_category = gsub("to low ", "to low\n", top_category), top_category = gsub(" Precedence", "\nPrecedence", top_category)) 

tractability_plot <- ggplot(tract_plot_data, aes(x=modality, y = n, fill = top_category, label = ifelse(n>35, paste0(top_category, " (", n, ")"), ""))) + geom_col() + geom_text(colour = "white", position = position_stack(vjust = 0.5), size = 3) + theme_bw() + theme(legend.position = "bottom", panel.grid=element_blank(), axis.title.x = element_blank()) + scale_y_continuous(expand = c(0, NA)) + scale_x_discrete(limits = c("smallmolecule", "antibody", "other_modalities"), labels = c("Small Molecule", "Antibody", "Other Modalities")) + guides(fill = FALSE) + labs(y = "Number of targets", subtitle = "TargetAge Target Tractability Assessment")

ggsave(tractability_plot, width = 5.9, height = 4.1, dpi = 300, file = "tractability_categories.png")
ggsave(tractability_plot, width = 5.9, height = 4.1, file = "tractability_categories.pdf")


targetage_annotations <- target_annotations %>% filter(targetId %in% targetage)

teps <- targetage_annotations %>% filter(!is.na(tep.uri))
probeminer <- targetage_annotations %>% select(targetId, chemicalProbes.portalprobes, chemicalProbes.probeminer.link) %>% filter(!is.na(chemicalProbes.portalprobes) | !is.na(chemicalProbes.probeminer.link))
probes <- targetage_annotations %>% unnest(chemicalProbes.portalprobes) %>% unnest(sourcelinks)

clinical_precedence <- targetage_tract %>% select(targetId, targetSymbol, tractability.smallmolecule.categories.clinical_precedence, tractability.antibody.categories.clinical_precedence, tractability.other_modalities.categories.clinical_precedence) %>% filter_at(vars(-targetId, -targetSymbol), any_vars(. > 0))


probable_targets <- targetage_tract %>% 
  ...
  select(targetId, targetSymbol, ends_with("top_category")) %>% 
  right_join(probes) %>% 
  left_join(top_cluster_genes %>% 
              filter(yProbaModel >= 0.5) %>% 
              select(targetId = gene.id, 
                     cluster, 
                     community, 
                     morbidities) %>% 
              unique(), by = c("targetId"))
...
```{r}
tmp <- tbl_genes %>% group_by(cluster) %>% filter(any(morbidity == "osteoarthritis")) %>% ungroup() %>% select(cluster, morbidity, gene.symbol, L2G) %>% group_by(cluster) %>% mutate(morbidities = paste0(unique(morbidity), collapse = "; ")) %>% ungroup() %>% group_by(cluster, morbidities, gene.symbol) %>% summarise(n_associations = length(gene.symbol), max_L2G = max(L2G) ) %>% arrange(cluster, -max_L2G, -n_associations) %>% filter(!is.na(gene.symbol))

tmp2 <- tmp %>% ungroup() %>% group_by(gene.symbol, max_L2G) %>% summarise(max_L2G = round(max(max_L2G),2), confidence = ifelse(max_L2G>=0.5, "higher", "lower"))  %>% unique()

write.table(tmp2, file = "oa_targets_summary.tsv", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
write.table(tmp, file = "oa_targets_clusters.tsv", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
...
us <- g_all$clusters %>% filter(n_morbidities>1) %>% ungroup() %>% select(-n_morbidities) %>% replace(is.na(.), 0)  %>% mutate(cluster = as.character(cluster)) %>% mutate_if(is.numeric, ~1 * (. > 0)) %>% data.frame()

# n with freq > 1 
n_intersects = 74

# filter out diseases only involved in combinations that show up once
all_d <- colnames(us)[2:ncol(us)]
freq_1_d <- c("gait.measurement", "alzheimers.disease.stroke", "lipid.measurement.lipid.measurement", "healthspan", "cartilage.thickness.measurement", "metabolic.syndrome.lipid.measurement", "lewy.body.dementia", "longevity", "atherosclerosis")
sets_to_plot <- all_d[! all_d %in% freq_1_d]

upset(us, sets = sets_to_plot, nintersects = n_intersects, order.by = c("freq"))
